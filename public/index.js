...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите название груза/пассажира');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название груза/пассажира', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Груз/пассажир '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Название судна:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Тип судна:<br>' + this.tlTime;
    ...
    button.innerHTML = 'Добавить груз/пассажира';
